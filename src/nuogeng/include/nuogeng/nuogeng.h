...
private:
	...
	ros::Publisher m_pub_gpsUtm;
	
	serial::Serial *m_serial_port;  // 串口指针

	boost::shared_ptr<boost::thread> m_read_thread_ptr; // 读串口的线程指针

	bool m_reading_status;  // 为真表示正在读
	uint8_t * const m_pkg_buffer;

	gps_msgs::gpsUtm m_gpsUtm;
};
